...
from moduli18.main1 import books_df, total_books, unique_titles, average_rating, average_price, top_titles, top_authors
...
with st.sidebar.form("book_form"):
    ...
    new_year = st.number_input("Year", min_value=2009, max_value=2002, step=1)
...
col2.metric("Unique Titles", top_titles)
...
with col2:
    ...
    top_authors = books_df['Author'].value_count().head(10)
    st.bar_chart(top_authors)


st.subheader("Genre Distributions")
fig = px.pie(books_df, names='Genre', title='Most Liked Genre(2009-2022)',color='Genre',
             color_discrete_sequence=px.colors.sequential.Plasma)
st.ploty_chart(fig)

st.subheader("Number of Fiction vs Non-Ficition Books Over the Years")
